			...
			>
				...
				<virtual-list :items="directoryList" :size="40" :remain="16" :active="curChapter.chapterIndex" :scrollHeight="windowHeight - 60">
					<template v-slot="{ item, active }">
						<view class="directory-listItem" :class="{ active: item.index == active }" @click="goToChapter(item.index)">{{ item.name }}</view>
					</template>
				</virtual-list>
			</view>
		</view>
	...
	methods: {
		...
		getOneChapter(chapterId, showLoading) {
			...
			return new Promise((resolve, reject) => {
				...
				let hastext = uni.getStorageSync('zw:' + this.bookId + ':' + chapterId);
				if (hastext) {
					...
					this.$u.api
						...
						.then(res => {
							if (res.success != 1) {
								return this.$u.toLocaleString(res.msg);
							}

							this.tmpChapter.text = res.chapter.content; //模拟数据
							if (this.simplified === 2) {
								//切换为繁体  注意：这里默认数据库中是简体字
								this.tmpChapter.text = traditionalized(this.tmpChapter.text);
							}
							uni.setStorage({
								key: 'zw:' + this.bookId + ':' + chapterId,
								data: res.chapter
							});
							resolve();
						})
